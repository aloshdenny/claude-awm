...
vol = modal.Volume.from_name("synthid-120b-weights", create_if_missing=True)
results = modal.Volume.from_name("synthid-120b-results", create_if_missing=True)
...
@app.function(image=image, gpu="H100", volumes={"/weights": vol, "/out": results},
              timeout=60 * 60 * 4)
def generate(domain: str, target: int = 2048):
    ...
    out_path = f"/out/docs_120b_{domain}.json"
    ...
    model = AutoModelForCausalLM.from_pretrained(
        MODEL, device_map="cuda:0", dtype="auto").eval()
...
@app.function(image=image, gpu="H100", volumes={"/out": results}, timeout=60 * 60)
def score():
    ...
    eos = tok.eos_token_id
    out = {}
    for f in sorted(glob.glob("/out/docs_120b_*.json")):
        dom = f.split("docs_120b_")[1].replace(".json", "")
    ...
    json.dump(out, open("/out/res_120b.json", "w"), indent=1)
    results.commit()
    return out
